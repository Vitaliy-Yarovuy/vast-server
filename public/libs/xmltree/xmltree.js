...
(function ($) {
    ...
    XMLTree = function (jdo, subTreeRequest) {
        ...
        var error;
        if (!jdo.fpath && !jdo.xml) error = "neither XML nor path to XML file passed"; else if ((!jdo.container || !$(jdo.container).length) && !jdo.justReturn) error = "No container selector passed or does not match element in DOM";
        if (error) {
            alert('XMLTree error - ' + error);
            return;
        }

        //some vars
        var li, appendTo, attrLI, container = $(jdo.container), rand = Math.floor(Math.random() * 10000000), thiss = this;
        ...
        //load file
        if (jdo.fpath)
            $.ajax({url: jdo.fpath, cache: jdo.cache == undefined ? true : jdo.cache, dataType: !jdo.jsonp ? 'xml' : 'jsonp'}).done(function (xml) {
                    thiss.xml = xml;
                    thiss.actOnXML(xml);
        ...
        //passed as string
            ...
            //rename tags
                this.xml = jdo.xml.replace(/<(\/)?(\w+)([^>]*)>/g, function ($0, $1, $2, $3) { return '<' + ($1 ? $1 : '') + $2 + '_' + rand + ($3 ? $3 : '') + '>'; }).replace(/<\?xml[^>]+>\s*/, '');
            this.actOnXML(jdo.xml);
        }

        this.jdo = jdo;
        this.rand = rand;
        this.subTreeRequest = subTreeRequest;
    ...
    XMLTree.prototype.actOnXML = function (xml) {
        ...
        var thiss = this, jdo = this.jdo, subTreeRequest = this.subTreeRequest, rand = this.rand;
        ...
        if (jdo.openAtPath) {
            var currSel = this.xml.find(jdo.openAtPath);
            if (currSel.length == 1) currSel.attr('currSel', 'true');
        }
    ...
    XMLTree.prototype.postBuild = function () {
        var thiss = this, jdo = this.jdo, subTreeRequest = this.subTreeRequest;
        ...
        if (jdo.subTreeBranches) {
            if (jdo.subTreeBranches === true)
                this.tree.addClass('subTreeRequestsOnAllNodes'); else
        ...
        this.tree.delegate('.plusMin', 'click', function (evt) {
            ...
            var currState = uls.filter(':hidden').length || !uls.length ? 'closed' : 'open', xPathToNode = returnXPathToNode($(this).parent()), li = $(this).parent();
        ...
        if (jdo.clickCallback)
            this.tree.delegate('.LIText', 'click', function (evt) {
                ...
                jdo.clickCallback(li, returnXPathToNode(li), evt);
            });
